...
postos_layout = {
    ...
    "CONTRATOS - CNX36": [1, 1, 1, 1, 1, 1],
    "CONTRATOS - FAST PASS:": [1, 1, 1, 1, 1, 1],
    "CONTRATOS - MERCADORIAS": [1, 1, 1, 1, 0, 0]
}

if "roster_grid" not in st.session_state or st.session_state.get("last_turno") != target_turno:
    postos = list(postos_layout.keys())
    default_data = {"Posto": postos}
    
    for i in range(1, 7):
        default_data[f"FUNÇÃO {i}"] = ["" if postos_layout[posto][i-1] else "-" for posto in postos]
...
if st.button("🔄 Escala Aleatória", help="Preenche as posições aleatoriamente com a lista de funcionários"):
        ...
        for i in range(1, 7):
            func_list = []
            for posto in postos:
                is_active = postos_layout[posto][i-1]
                if is_active:
                    if i == 6 and lideres:
                        emp = lideres.pop()
...
if st.button("Salvar Escala", type="primary"):
    ...
    session.commit()
    st.success(f"Escala do turno {target_turno} para {data_escala} salva com sucesso como {status}!")
